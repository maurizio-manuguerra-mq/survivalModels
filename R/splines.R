...
basis2_mpl = function(x, knots, order, which=c(1,2,3),  outer.ok = F, splines=c("bsplines", "isplines")){
  ...
  if (splines=="bsplines"){
    ...
    if (nrow(M_Psi_nm)>1) M_Psi_nm = M_Psi_nm - matrix(apply(M_Psi_nm,2,mean),nrow=n,ncol=m,byrow = T)
    ...
    M_Psi_nm   = PSIs[[1]][,3:(m+2),drop=F]
    if (nrow(M_Psi_nm)>1) M_Psi_nm   = M_Psi_nm - matrix(apply(M_Psi_nm,2,mean),nrow=n,ncol=m,byrow = T)
    M_psi_nm   = PSIs[[2]][,3:(m+2),drop=F]
    M_psi22_nm = PSIs[[3]][,3:(m+2),drop=F]
  } else {
    stop("\nSplines not supported.\n\n")
  }
