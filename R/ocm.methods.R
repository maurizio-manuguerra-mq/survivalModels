...
predict.ocm <- function(object, newdata=NULL, type= c("response", "density", "CDF", "quantile", "regressor", "exp_regressor", "hazard", "cum_hazard", "survival"), prob = 1:(K - 1) / K, K = 50, CI = NULL, ...)
{
  type <- match.arg(type)
  if (length(CI) & type != "response") stop('Std errors are currently supported ony for type "response"')
  if (is.null(newdata)){
  ...
  ##
  if (type=="response"){
    # print(pars_obj[[1]]$mat)
    ## Depends on original gfun and then v
    # h <- curval$h
    # hstar <- h - gfun(pars_obj, intercept=TRUE)
    ## Does not depends on original gfun and then v
    hstar <- lin_regressor(pars_obj, gfun = F, intercept = F)
    out <- gfun_inv(-hstar, pars_obj) 
    # 95% CIs
    if (length(CI)){
      fix_index = which(sapply(pars_obj, function(x)x$type)=="fix.eff")
      if (!(CI %in% names(pars_obj[[fix_index]]$pars))) stop("CI: variable name not recognised.")
      p <- length(object$pars_obj[[1]]$pars) -1 # intercept
      # Work on copies of pars_obj
      se <- sqrt(diag(object$vcov))
      pars_obj_inf <- pars_obj_sup <- pars_obj
      pars_obj_inf[[fix_index]]$pars[CI] <- pars_obj[[fix_index]]$pars[CI] + 1.96*se[CI]
      pars_obj_sup[[fix_index]]$pars[CI] <- pars_obj[[fix_index]]$pars[CI] - 1.96*se[CI]
      hstar_inf <- lin_regressor(pars_obj_inf, gfun = F, intercept = F)
      hstar_sup <- lin_regressor(pars_obj_sup, gfun = F, intercept = F)
      out=cbind(response=out, CIinf=gfun_inv(-hstar_inf, pars_obj), CIsup=gfun_inv(-hstar_sup, pars_obj))
    }
...
##############################
#' @export
predict_response_with_CI <-function(object, newdata=NULL, CIs=T, ...)
{
  # type <- match.arg(type)
  if (is.null(newdata)){
    pars_obj <- object$pars_obj
  ...
  ## Alternative
  ## ordinalCont:::lin_regressor(pars_obj) - ordinalCont:::gfun(pars_obj)
  h <- curval$h
  hstar <- h - as.numeric(gfun(pars_obj, intercept=TRUE))
  out <- gfun_inv(-hstar, pars_obj) ## WARNING: the std gfun_inv won't work because it uses gfun, which relies on pars_obj[[gfun_index]]$mat  %*% pars_obj[[gfun_index]]$pars. pars_obj[[gfun_index]]$mat rows are all the same
  
  if (CIs) {
    out_low = gfun_inv(-hstar, pars_obj)
    out_up  = gfun_inv(-hstar, pars_obj)
  }
  out = rbind(out, out_low, out_up)
  ...
  ###
{
  ...
  forms <- sapply(mlist, function(x) deparse(x$call$formula, width.cutoff = 500))
