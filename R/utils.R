...
ocmPars_update <- function(object, newdata, include_gfun = TRUE){
  ...
  ## Join with newdata
  #assume the structure of data and newdata are identical
  # internames <- intersect(names(data), names(newdata))
  # FIXME: if there are other columns beyon internames, the following does not work
  # data[internames] <- newdata[internames]
  data <- newdata
  # FIXME: run differently on numerical and categorical vars; also problem with everything being converted to text.
  ...
  if (length(v)>1) v <- ((length(v)-1)*v+0.5)/length(v)
  ...
  mf.fix <- model.frame(formula=form.fix, data=data)
  #CHECK
  # WARNING: update mat in pars_obj !!!!! (2021.04.08)
  # x <- mf.fix
  x <- model.matrix(attr(mf.fix, "terms"), data=mf.fix)
  ...
  if (include_gfun) {
    ...
    obj = append(obj, pars_obj_fit[[gfun_ind]])
  }
  ## if include_gfun is FALSE, the original observed scale is used in ther gfun slot. Useful when used for prediction and gfun_inv()
  ############
  #RND effects
  ############
  nrndterms <- length(i_rnd)
...
predict.g.spline <- function(par_obj, newvalues=NULL){
  ...
  vars=apply(mat,2,var)
  idrop=par_obj$idrop
  mat=mat[,-idrop, drop=F]
  mat1=mat1[,-idrop, drop=F]
  mat2=mat2[,-idrop, drop=F]
